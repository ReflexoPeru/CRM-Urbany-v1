...
### 7. Iniciar con Docker (Recomendado)

Si prefieres usar Docker para el desarrollo local:
...
- **Redis**: Para mensajería y Celery.
- **Celery Worker/Beat**: Procesamiento en segundo plano.
...
El repositorio sigue una estrategia de 2 ramas principales:

1. **`main`**: Rama estable para desarrollo local. Usa el archivo `docker-compose.yml`.
2. **`production`**: Rama lista para el VPS. Incluye configuraciones de seguridad, `WhiteNoise` y puertos optimizados para el VPS.
